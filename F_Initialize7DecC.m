function [People Mossies]= F_Initialize7DecC(ND,NP,NM,N0,D1,D2,D3);
% 
%   Initializes arrays People & Mossies from the transport matrices
%   Version B= No mossies at Zona Sul station
%   Version C = No mossies at Centro station
%   Version BC= No mossies at Zonal Sul or Centro stations
%   
% INPUT
...
% NM mosquitos in each location (except possibly ZonaSul station)
    ...
    M2=[6*ones(NM(6),1);7*zeros(NM(7),1);8*ones(NM(8),1);9*ones(NM(9),1);10*ones(NM(10),1)];
    Mossies(:,1)=[M1;M2];
